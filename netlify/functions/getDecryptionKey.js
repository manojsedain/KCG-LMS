// netlify/functions/getDecryptionKey.js - Deliver AES decryption key to validated devices
    ...
    try {
        ...
        if (!device) {
            await db.createLog({
                log_type: 'security',
                level: 'warn',
                message: 'Decryption key request for unknown device',
        ...
        if (device.username !== username) {
            await db.createLog({
                log_type: 'security',
                level: 'error',
                message: 'Decryption key request with mismatched username',
                details: { 
                    device_id: device.id,
        ...
        if (device.status !== 'active') {
            await db.createLog({
                log_type: 'security',
                level: 'warn',
                message: 'Decryption key request from inactive device',
                details: { 
                    device_id: device.id, 
                    username,
        ...
        if (new Date(device.expires_at) < new Date()) {
            ...
            await db.createLog({
                log_type: 'device',
                level: 'info',
                message: 'Expired device attempted key access',
        ...
        // Log successful key delivery
        await db.createLog({
            log_type: 'security',
            level: 'info',
            message: 'Decryption key delivered',
            details: { 
                device_id: device.id, 
                username 
            },
        ...
        console.error('Decryption key delivery error:', error);

        await db.createLog({
            log_type: 'error',
            level: 'error',
            message: 'Decryption key delivery error',
            details: { 
                error: error.message,
                stack: error.stack 
            },
